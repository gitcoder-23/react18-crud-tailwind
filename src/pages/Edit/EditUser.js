...
const EditUser = () => {
  ...
  const [userName, setUserName] = useState(state?.singleUser?.name || '');
  const [email, setEmail] = useState(state?.singleUser?.email || '');
  const [phone, setPhone] = useState(state?.singleUser?.phone || '');
  ...
  const editUserData = (e) => {
    ...
    if (!userName || !email || !phone) {
      ...
      axios
        .put(`${rootApi}/users/${state?.singleUser?.id}`, formData)
        .then((res) => {
          console.log('res->', res);
          setIsLoading(false);
          setIsError(false);
    ...
    <>
      <div className="w-screen h-full flex flex-col px-14 py-8  justify-center items-center mt-16 ">
        ...
        <div className="w-[80%] block p-6 rounded-lg shadow-lg bg-white items-center justify-center">
          ...
          <form className=" h-full flex flex-col  items-center mt-4 ">
            <input
              value={userName}
